...
> 需要在**装有游戏和卫月的 Windows 电脑**上编译（要引用卫月注入后生成的 Dalamud 开发程序集），Mac 上只能改代码。
>
> Must be built on the **Windows machine with the game and Dalamud CN installed** (the build references the dev assemblies generated by Dalamud injection).
...
- 本项目基于 `Dalamud.NET.Sdk/15.0.0`（对应卫月 Dalamud 15 / .NET 10）。如编译报 API 或运行时版本不匹配，把 csproj 中的 SDK 版本改成与当前卫月一致的版本；若信息栏条目的 `OnClick` 编译报错（不同 API 版本签名不同），按 `Plugin.cs` 中注释改为 `dtrEntry.OnClick = _ => Toggle();` 即可。
