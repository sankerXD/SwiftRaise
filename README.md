...
- 本项目基于 `Dalamud.NET.Sdk/12.0.2`（API 12）。如编译报 API 不匹配，把 csproj 中的 SDK 版本改成与当前卫月一致的版本；若信息栏条目的 `OnClick` 编译报错（不同 API 版本签名不同），按 `Plugin.cs` 中注释改为 `dtrEntry.OnClick = _ => Toggle();` 即可。
